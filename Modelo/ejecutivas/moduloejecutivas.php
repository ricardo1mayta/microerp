...
and concat(e.EMP_D_RAZONSOCIAL,' ',e.EMP_D_NOMBRECOMERCIAL) LIKE '".$txt."%' GROUP BY e.EMP_C_CODIGO ORDER BY e.EMP_C_CODIGO DESC limit $ini,$intervalo; ";
         $rows=$this->db->query($sql);  
        return $rows;
    }
 ...
 and concat(e.EMP_D_RAZONSOCIAL,' ',e.EMP_D_NOMBRECOMERCIAL) LIKE '$txt%' GROUP BY e.EMP_C_CODIGO ORDER BY e.EMP_C_CODIGO DESC limit 10;";
         $rows=$this->db->query($sql);  
        return $rows;
    }
    ...
    {
       $sql = "SELECT count(e.EMP_C_CODIGO) FROM dg_cartera c INNER JOIN dg_empresas e on c.EMP_C_CODIGO=e.EMP_C_CODIGO WHERE c.US_C_CODIGO='$us' AND e.EMP_E_ESTADO=1 and concat(e.EMP_D_RAZONSOCIAL,' ',e.EMP_D_NOMBRECOMERCIAL) LIKE '$txt%' ;";
         $rows=$this->db->query($sql);  
        $result=$rows->fetch_array();
