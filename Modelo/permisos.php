...
public function __construct() 
{
   ...
   public function chexview($uid,$vista) {
    $res="";
      $sql2="SELECT VIS_L_ENLACE FROM dg_vista_menu WHERE US_C_CODIGO='$uid' and PRM_E_ESTADO>0;";
            $rows2=$this->db->query($sql2); 
            while($row=$rows2->fetch_array()){
                            
              $vis=$row['VIS_L_ENLACE'];
              if($vis==$vista){
                $res=$vis;
                 } 
    ...
    public function user_menu1($idus,$idpad) {
        $sql="select  v.VIS_C_CODIGO,v.VIS_D_NOMBRE from dg_permisos p inner join dg_vistas v on v.VIS_C_CODIGO = p.VIS_C_CODIGO WHERE p.US_C_CODIGO='$idus' AND p.PRM_E_ESTADO=1 AND v.VIS_P_PADRE='$idvis'";
    ...
    public function user_sub_menu1($idus,$idpad) {
        $sql="select  v.VIS_C_CODIGO,v.VIS_D_NOMBRE from dg_permisos p inner join dg_vistas v on v.VIS_C_CODIGO = p.VIS_C_CODIGO WHERE p.US_C_CODIGO='$idus' AND p.PRM_E_ESTADO=1 AND v.VIS_P_PADRE='$idpad'";
    ...
    public function user_sub_menu2($idus,$idpad,$activ) {
      ...
      <ul class="treeview-menu">
               <?php  while($lista=$result->fetch_array()){ 
                        if($lista['VIS_L_ENLACE']==$activ){
                          ?>
                          <li class="active"><a href="<?php echo '../'.$lista['VIS_L_ENLACE'].'/'; ?>" target="_self"><i class="fa fa-check-circle"></i><?php echo $lista['VIS_D_NOMBRE']; ?> </a>
                          </li>       
