...
class DetProducto {
           ...
           /* public function get_productos() 
            ...
            public function guardar_detproductos_ventas($nombre,$cantidad,$precio,$idprod) 
            {
                $sql="call sp_insertar_dedproducto_ventas('".$nombre."','".$cantidad."','".$precio."','".$idprod."')";
                $res = mysql_query($sql) or die(mysql_error());
                $no_rows = mysql_fetch_array($res);
                return $no_rows;
            }
            ...
            {
               $sql="select pb.PAR_D_NOMBRE,pa.PAR_D_NOMBRE,p.PRO_D_NOMBRE,dp.DPRO_D_NOMBRE from dg_detalleProductos dp
            ...
            public function get_AllDetalleproductos() 
            {
               /* $sql="select pb.PAR_D_NOMBRE,pa.PAR_D_NOMBRE,p.PRO_D_NOMBRE,dp.DRPO_D_NOMBRE from dg_detalleProductos dp
                        ...
                        INNER JOIN dg_parametros pa on pa.PAR_C_CODIGO=p.PAR_C_CODIGO
                        INNER JOIN dg_parametros pb on pb.PAR_C_CODIGO=pa.PAR_C_PADRE;";
                $sql="select pb.PAR_D_NOMBRE,pa.PAR_D_NOMBRE,p.PRO_D_NOMBRE from dg_productos p
                        INNER JOIN dg_parametros pa on pa.PAR_C_CODIGO=p.PAR_C_CODIGO
                        INNER JOIN dg_parametros pb on pb.PAR_C_CODIGO=pa.PAR_C_PADRE;";
                $res = mysql_query($sql);
                return $res;
