...
private $db;
    ...
    public function save_detproducto($idpro,$nombre,$cantidad,$precio) 
    {  
       $sql = "CALL spa_save_detproducto('$idpro','$nombre','$cantidad','$precio');";
    ...
    {  
     ...
     public function get_Detalleproductos() 
            {
               $sql="select p.PRO_C_CODIGO,p.PRO_D_NOMBRE,dp.DPRO_D_NOMBRE,dp.DPRO_C_CODIGO,dp.DPRO_N_CANTIDAD,dp.DPRO_N_PRECIO from dg_detalleProductos dp
                        inner join dg_productos p on p.PRO_C_CODIGO=dp.PRO_C_CODIGO
                        INNER JOIN dg_parametros pa on pa.PAR_C_CODIGO=p.PAR_C_CODIGO
                        INNER JOIN dg_parametros pb on pb.PAR_C_CODIGO=pa.PAR_C_PADRE  where dp.DPRO_E_ESTADO=1 order by dp.DPRO_C_CODIGO DESC;";
