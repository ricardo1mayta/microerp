			...
			switch ($evento) {
				case 'guardar':
						if( !empty($_POST['rsocial'])  and !empty($_POST['ruc'])){
								 ...
								 $prod=new Moduloejecutivas();
								  

								$p=$prod->solicitar_empresa($ruc,$rsocial,$nomcomercial,$idus);
									if($p['sms']=='ok'){
										if($id!=""){
	                          ...
	                          include(HTML_DIR .'ejecutivas/listarEmpresas.php');
				...
				case 'editar':
						...
						if(!empty($_POST['codigo']) and !empty($_POST['rsocial1']) and !empty($_POST['ruc1'])){
									...
									$p=$prod->editar_empresa($codigo,$nombre,$ruc,$rsocial,$idus);
										if($p['sms']=='ok'){
